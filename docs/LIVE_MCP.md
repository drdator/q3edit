...
- `map_compile` runs the live map through q3map and then BSPC at fast, normal, or full quality. Warnings and errors are structured by severity and linked to implicated references when texture names or entity origins make that possible. Pass a `.bsp` `artifactPath` to atomically write the BSP and, when generation succeeds, its sibling `.aas`; `map_save_and_compile` supports the same option.
- Compile results report BSP, VIS, LIGHT, and AAS status independently. Fast compilation skips VIS/LIGHT but still generates bot navigation. WASM memory failures are classified as `compiler-memory` and include the active pass, so a LIGHT or AAS failure is distinguishable from a BSP/VIS failure that prevents any playable artifact.
...
- `map_play` compiles and launches the current revision in browser ioquake3, installs its generated AAS beside the BSP, and supports optional verified noclip. After `map_compile` or `map_save_and_compile`, pass `useLastCompile: true` to launch the cached BSP/AAS pair for the unchanged revision without compiling again. `game_status` reports whether bot navigation was installed, and `game_screenshot` captures the running compiled/lightmapped view.
