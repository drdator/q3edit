...
export function collectEditorDiagnostics(editor: Editor): EditorDiagnostic[] {
  ...
  editor.entities.forEach((entity, entityIndex) => {
    ...
    else if (!getEntityClassRegistry().get(entity.classname)) result.push({ severity: 'info', code: 'unknown-class', message: `${entityId(entityIndex)} uses undocumented class ${entity.classname}`, target });
