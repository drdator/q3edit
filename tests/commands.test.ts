...
describe('CommandRegistry', () => {
  ...
  it('exposes checked state for display categories, renderer modes, and lighting', () => {
    ...
    registry.execute('view.renderer.editor-fill');
    expect(registry.getState('view.renderer.editor-fill').checked).toBe(true);
    expect(registry.getState('view.renderer.editor-fill').label).toBe('Editor Fill');
    ...
    registry.execute('view.camera-isometric.northeast');
    expect(editor.cameraProjection3d).toBe('orthographic');
    expect(registry.getState('view.camera-projection.orthographic').checked).toBe(true);
